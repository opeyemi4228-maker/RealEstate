 ...
 *  - EMAIL_FROM          e.g. "Real Estate <noreply@realestate.com>"
 *                        (the domain must be verified in Resend)
 *  - EMAIL_TO            internal inbox for notifications, e.g.
 *                        info@realestate.com
 */

const FROM = process.env.EMAIL_FROM || "Real Estate <onboarding@resend.dev>";
const TEAM_INBOX = process.env.EMAIL_TO || "info@realestate.com";
...
const row = (label, value) =>
  value
    ? `<tr><td style="padding:4px 12px 4px 0;color:#6b7280;font-size:13px;">${esc(
        label
      )}</td><td style="padding:4px 0;color:#0A1A36;font-size:13px;font-weight:600;">${esc(
        value
      )}</td></tr>`
    : "";
...
const shell = (heading, bodyHtml) => `
  <div style="font-family:Arial,Helvetica,sans-serif;max-width:560px;margin:0 auto;border:1px solid #eee;border-radius:8px;overflow:hidden;">
    <div style="background:#0A1A36;padding:20px 24px;">
      <span style="color:#FFC72C;font-weight:700;letter-spacing:.18em;font-size:11px;text-transform:uppercase;">Real Estate</span>
    </div>
    <div style="padding:24px;">
      <h2 style="margin:0 0 16px;color:#0A1A36;font-size:18px;">${esc(heading)}</h2>
      ${bodyHtml}
    </div>
  </div>`;
...
export function notifyContactMessage(msg) {
  const html = shell(
    ...
    <p style="margin:0;color:#0A1A36;font-size:14px;line-height:1.6;white-space:pre-wrap;">${esc(
      msg.message
    )}</p>`
  );
  return send({
...
export function confirmContactMessage(msg) {
  const html = shell(
    ...
    `<p style="margin:0 0 12px;color:#0A1A36;font-size:14px;line-height:1.7;">
     ...
     <p style="margin:0;color:#6b7280;font-size:13px;line-height:1.7;">
      If your enquiry is urgent, email us directly at
        <a href="mailto:info@realestate.com" style="color:#0A1A36;">info@realestate.com</a>.</p>`
  );
  return send({
      to: msg.email,
      subject: "We've received your message — Real Estate",
    html,
  });
}
